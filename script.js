...
// ===== 2. MULTI-LANGUAGE SYSTEM (BUGS FIXED) =====
const resources = {
    id: {
        translation: {
...
function initCursor() {
    ...
    const hoverElements = document.querySelectorAll('a, button, select, [class*="btn"], .glass, .magnetic-target');
    hoverElements.forEach(el => {
        el.addEventListener('mouseenter', () => { cursor.classList.add('scale-[3]', 'bg-cyan-400'); cursor.classList.remove('bg-white'); });
        el.addEventListener('mouseleave', () => { cursor.classList.remove('scale-[3]', 'bg-cyan-400'); cursor.classList.add('bg-white'); });
    });
}
...
function initMagneticElements() {
    ...
    magnets.forEach((magnet) => {
        magnet.addEventListener('mousemove', (e) => {
            ...
            // Jitter-fix: Multiply by 0.1 agar tidak bergetar saat hover di ujung kotak
...
// ===== 10. COMPARISON SLIDER (Visual Impact Tool) =====
